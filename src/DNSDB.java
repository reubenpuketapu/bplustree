...
import bplusinttostring.BPlusTreeIntToString60;
import bplusinttostring.IntToStringLeafNode;
import bplusstringtoint.BPlusTreeString60toInt;
...
public class DNSDB {
	...
	public void iterateAll() {
		...
		IntToStringLeafNode node = hostNames.findFirst();

		while (node != null) {
			for (Map.Entry<Integer, String> value : node.values.entrySet()) {
				System.out.println(IPToString(value.getKey()) + "\t"
						+ value.getValue());
			}
			node = node.next;
		}

		// hostNames.iterateInternal((InternalNode) hostNames.BRoot);
